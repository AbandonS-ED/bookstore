...
| password | VARCHAR(64) | NOT NULL | 密码（BCrypt加密） |
...
| 字段名 | 类型 | 约束 | 说明 |
|--------|------|------|------|
| id | BIGINT | PK | 书籍ID（雪花算法，非 AUTO_INCREMENT） |
...
| 字段名 | 类型 | 约束 | 说明 |
|--------|------|------|------|
| id | BIGINT | PK | 评论ID（雪花算法） |
| user_id | BIGINT | FK, NOT NULL | 用户ID |
| book_id | BIGINT | FK, NOT NULL | 书籍ID |
| order_item_id | BIGINT | FK | 关联订单项ID（已购买用户才能评论） |
...
### 2.13 社区帖子表 (community_post)

| 字段名 | 类型 | 约束 | 说明 |
|--------|------|------|------|
| id | BIGINT | PK | 帖子ID（雪花算法） |
| user_id | BIGINT | FK | 发布用户ID |
| username | VARCHAR(50) | | 用户名 |
| content | VARCHAR(500) | | 帖子内容 |
| image_url | VARCHAR(500) | | 图片URL |
| likes | INT | DEFAULT 0 | 点赞数 |
| liked | TINYINT | DEFAULT 0 | 是否已点赞 |
| book_id | BIGINT | FK | 关联书籍ID（可选） |
| create_time | DATETIME | DEFAULT CURRENT_TIMESTAMP | 创建时间 |
| update_time | DATETIME | ON UPDATE CURRENT_TIMESTAMP | 更新时间 |

### 2.14 社区点赞表 (community_like)
...
user (1) ────── (n) community_post ────── (0..1) book
user (1) ────── (n) community_like ────── (1) community_post
```
