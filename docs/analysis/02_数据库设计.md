...
|------|------|
| `user` | 用户表 |
| `book` | 书籍表（含 orig_price 划线原价、quote 名言） |
| `category` | 分类表 |
| `cart` | 购物车表 |
| `order` | 订单表 |
| `order_item` | 订单明细表 |
...
创建 → 支付中 → 已支付 → 已发货 → 已收货 → 已完成
（created/paying 可取消/cancelled，created 可过期/expired，paid/shipped 可退款/refunded）
...
CREATE TABLE `book` (
    ...
    `orig_price` DECIMAL(10,2) NULL COMMENT '划线原价',
    `stock` INT DEFAULT 0,
    `sales` INT DEFAULT 0,
    `category_id` BIGINT,
    `description` TEXT,
